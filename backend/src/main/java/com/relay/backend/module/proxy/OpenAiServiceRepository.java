...
public class OpenAiServiceRepository {
  ...
  public OpenAiServiceConfig save(String apiEndpoint, String token) {
    return jdbcTemplate.queryForObject(
        ...
        values (?, ?)
        returning id, api_endpoint, token, force_replace_codex_model, codex_replacement_model, created_at, updated_at
        """,
        this::mapServiceRow,
        apiEndpoint,
        token);
  ...
  public java.util.List<OpenAiServiceConfig> findAll() {
    return jdbcTemplate.query(
        ...
        select
          ...
          s.api_endpoint,
          s.token,
          s.force_replace_codex_model,
          s.codex_replacement_model,
          s.created_at,
          s.updated_at,
          p.id as profile_id,
          p.profile_name,
  ...
  private OpenAiServiceConfig mapRow(ResultSet rs, int rowNum) throws SQLException {
    ...
    return new OpenAiServiceConfig(
        ...
        rs.getString("token"),
        rs.getBoolean("force_replace_codex_model"),
        rs.getString("codex_replacement_model"),
